<?php
/**
 * @brief		X PayNow Checkout ACP License Controller
...
class _license extends \IPS\Dispatcher\Controller
{
	...
	public function manage(): void
	{
		...
		$status   = \IPS\xpaynowcheckout\License\Checker::getStatus();
		$cache    = \IPS\xpaynowcheckout\License\Checker::getCacheData();
		$licKey   = (string) \IPS\Settings::i()->{'xenntec_lic_xpaynowcheckout_key'};
		...
		\IPS\Output::i()->title  = \IPS\Member::loggedIn()->language()->addToStack( '__app_xpaynowcheckout_license' );
		\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'xenntec', 'xpaynowcheckout', 'admin' )->license(
			$status,
			$cache,
			$maskedKey,
			$licKey !== ''
	...
	protected function _activate(): void
	{
		...
		$status = \IPS\xpaynowcheckout\License\Checker::activate( $key );

		$flashMsg = match ( $status ) {
			\IPS\xpaynowcheckout\License\Checker::STATUS_VALID,
			\IPS\xpaynowcheckout\License\Checker::STATUS_EXPIRING => \IPS\Member::loggedIn()->language()->addToStack( 'xenntec_lic_activated_ok' ),
			default => \IPS\Member::loggedIn()->language()->addToStack( 'xenntec_lic_activated_fail' ),
		};

		\IPS\Output::i()->redirect(
			\IPS\Http\Url::internal( 'app=xpaynowcheckout&module=xenntec&controller=license' ),
			$flashMsg
		);
	}
	...
	protected function _recheck(): void
	{
		...
		\IPS\xpaynowcheckout\License\Checker::clearCache();
		\IPS\xpaynowcheckout\License\Checker::getStatus();
